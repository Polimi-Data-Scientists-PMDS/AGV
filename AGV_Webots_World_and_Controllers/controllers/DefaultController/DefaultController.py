...
class AGVSimulation:
    ...
    def run(self):
        ...
        try:
            while self.hardware.is_alive():
                ...
                # --- 2. CHECK GOAL ---
                # before updating state (1-tick delay) -> if reached it changes it
                self.__check_goal_reached(current_time)
                ...
                if self.__should_print(current_time):
                    self.__print_data(current_time, dist_e, heading_e, lin_vel, ang_vel)
    ...
    def __should_log(self, current_time):
        return current_time - self.last_db_save >= self.config.LOG_INTERVAL_SEC
