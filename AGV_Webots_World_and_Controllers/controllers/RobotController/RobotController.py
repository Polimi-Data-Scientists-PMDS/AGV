...
class State:
    ...
    def calculate_errors(self, goal: Position):
        ...
        """
        if goal is None:
            raise Exception("Goal position is None")
        dx = goal.x - self.x
        dy = goal.y - self.y
...
class RobotController:
    ...
    def calculate_velocity(self, rho, alpha):
        ...
        lin_vel = min(lin_vel, self.MAX_LIN_VELOCITY)   # max forward speed (tune for your robot)
        # angular velocity limits  
        ang_vel = min(self.MAX_ANG_VELOCITY, ang_vel)
        ang_vel = max(-self.MAX_ANG_VELOCITY, ang_vel)
